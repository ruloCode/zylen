...
/** Raw payload of the get_user_stats RPC (returns Json). */
interface UserStatsRpc {
  active_days?: number;
...
export class StatsService {
  ...
  static async getUserStats(): Promise<UserStats> {
    try {
      ...
      // Get user stats via RPC (typed as Json — narrow to the known payload)
      const { data, error } = await supabase.rpc('get_user_stats', {
        p_user_id: userId,
      });
      const stats = (data ?? {}) as UserStatsRpc;
  ...
  static async getXPDistribution(): Promise<XPDistribution[]> {
    try {
      ...
      return (
        (data as unknown as any[] | null)?.map((item: any) => ({
  ...
  static async getCompletionTrend(days = 30): Promise<CompletionTrendPoint[]> {
    try {
      ...
      return (
        (data as unknown as any[] | null)?.map((point: any) => ({
