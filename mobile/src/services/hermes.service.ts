 ...
 * which we persist per device. The `streamChatCompletion` signature mirrors
 * the OpenAI service so both backends stay interchangeable.
 *
 * The app reaches the local Hermes machine through an HTTPS tunnel, so
 * EXPO_PUBLIC_HERMES_API_URL points at the tunnel base (…/v1). Locally it
 * defaults to http://localhost:8642/v1. These EXPO_PUBLIC_* vars are inlined
 * at build time, so changing them requires a fresh build.
 *
 * Streaming: React Native's built-in fetch cannot expose a ReadableStream
 * body, so we use `expo/fetch` (WinterCG-compliant) whose response.body
 * supports `getReader()` for incremental SSE consumption.
 *
 * Gateway contract:
 *   POST {HERMES_API_URL}/chat/completions
...
const HERMES_API_URL = (ENV.HERMES_API_URL || 'http://localhost:8642/v1').replace(/\/$/, '');

const HERMES_API_KEY = ENV.HERMES_API_KEY || '';

const SESSION_STORAGE_KEY = 'everlight_hermes_session_id';

/**
 * Returns a stable session id for this device/user, creating and persisting
 * one on first use so Hermes can keep conversation memory across restarts.
 */
...
function resetSession(): void {
  try {
    kv.removeItem(SESSION_STORAGE_KEY);
  } catch {
    /* noop */
  }
}
...
export async function streamChatCompletion(
  ...
  const response = await expoFetch(`${HERMES_API_URL}/chat/completions`, {
    method: 'POST',
    headers: {
