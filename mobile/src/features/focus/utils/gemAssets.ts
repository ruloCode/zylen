 ...
 *
 * RN port: the path helpers keep returning the web-style public/ paths
 * ('/gems/…'); resolve them to bundled assets with `img()` from
 * `@/assets/registry` (or use the `*Source` helpers below) before passing
 * them to expo-image.
 */
...
/** Bundled-asset source for a gem stage sprite (expo-image `source`). */
export function gemStageImageSource(
  species: GemSpecies,
  stage: GemStage
): number | undefined {
  return img(gemStageImage(species, stage));
}
