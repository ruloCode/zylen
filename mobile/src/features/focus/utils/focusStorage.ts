 ...
 * anchored so an app restart or a backgrounded app never loses time; the
...
export function persistFocusSession(session: ActiveFocusSession): void {
  try {
    kv.setItem(FOCUS_CONFIG.storageKey, JSON.stringify(session));
  } catch (error) {
...
export function readPersistedFocusSession(): ActiveFocusSession | null {
  try {
    const raw = kv.getItem(FOCUS_CONFIG.storageKey);
    if (!raw) return null;
...
export function clearPersistedFocusSession(): void {
  try {
    kv.removeItem(FOCUS_CONFIG.storageKey);
  } catch {
    /* noop */
  }
}
