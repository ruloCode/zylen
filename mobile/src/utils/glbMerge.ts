/**
 * glbMerge — React Native port of ../src/utils/glbMerge.ts (itself a browser
 * port of everlight-game/tools/glb_merge_anims.py).
 *
 * Merges single-clip GLBs (Meshy animation outputs) into one multi-clip GLB
 * on top of the rigged base model, entirely in JS (zero deps, Hermes-safe).
 ...
 *
 * RN divergence from the web version: `serializeGlb`/`mergeHeroGlb` return
 * `Uint8Array` instead of `Blob` — RN's Blob has no .arrayBuffer(), and the
 * Supabase upload takes the raw bytes' ArrayBuffer directly.
 */
...
export function serializeGlb(json: GltfJson, bin: Uint8Array): Uint8Array {
  ...
  // Single exact-length output buffer (trailing bin padding stays zeroed).
  ...
  const binHeaderOff = 20 + jsonBytes.length;
  dv.setUint32(binHeaderOff, bin.length + binPad, true);
 ...
 */
export function mergeHeroGlb(baseGlb: ArrayBuffer, clips: GlbClipSource[]): Uint8Array {
