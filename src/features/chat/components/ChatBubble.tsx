...
export function ChatBubble({
  ...
  return <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'} mb-4`}>
      ...
      <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-[75%]`}>
        <div className={`rounded-2xl px-4 py-3 border ${isUser ? 'bg-teal-500/90 border-teal-600/30' : 'bg-white border-parchment-300/50'}`}>
          <p className={`text-sm leading-relaxed ${isUser ? 'text-white' : 'text-gray-900'}`}>{message}</p>
        </div>
