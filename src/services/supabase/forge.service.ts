 ...
 *                           4 GLBs are re-hosted, merges them IN THE BROWSER
...
import { composeRigSheet, type ImagePart } from '@/utils/avatarImage';
...
export class ForgeService {
  ...
  static async generateRigSheet(forge: HeroForge, gender?: Gender): Promise<void> {
    ...
    const { assetToImagePart } = await import('@/utils/avatarImage');
    ...
    const sheetBlob = await composeRigSheet((data as { rig: ImagePart }).rig);
    const path = `${userId}/forge/${forge.id}/rig.png`;
    const { error: uploadError } = await supabase.storage
      .from('avatars')
      .upload(path, sheetBlob, { contentType: 'image/png', upsert: true });
    if (uploadError) {
  ...
  /** Download the 4 re-hosted GLBs, merge them in the browser, upload the
