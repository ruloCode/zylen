 ...
 *
 * Dev bypass: with VITE_SKIP_AUTH the whole service operates on a
 * localStorage blob so the feature demos offline. The slice and UI stay
...
/* ------------------------------------------------------------------------ */
/* Dev (VITE_SKIP_AUTH) adapter — localStorage-backed                        */
/* ------------------------------------------------------------------------ */
...
function devLoad(): DevBlob {
  try {
    const raw = localStorage.getItem(FOCUS_CONFIG.devStorageKey);
...
function devSave(blob: DevBlob): void {
  localStorage.setItem(FOCUS_CONFIG.devStorageKey, JSON.stringify(blob));
}
...
export class FocusService {
   ...
   * adapter mirrors them against the localStorage session ledger.
   */
